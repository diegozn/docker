...
public class TelaPrincipal extends javax.swing.JFrame {
    ...
    List<Totem> verificacaoTotemSql = connect.getJdbcSql().query(String.format("SELECT * FROM totem WHERE id_processador='%s' AND serial_disco='%s' AND hostname='%s'",comps.getIdProcessador(), comps.getSerialDisco(), comps.getHostname()), new BeanPropertyRowMapper<>(Totem.class));
...
//        setExtendedState(MAXIMIZED_BOTH);
    ...
    void enviarRelatorio(Relatorio relat){
      connect.getJdbc().update("INSERT INTO logs(fkTotem,pctg_processador,pctg_memoria_uso,pctg_disco_uso,qtd_processos,qtd_servicos,temp,data_hora) VALUES (?,?,?,?,?,?,?,?)", relat.getFkTotem(),relat.getPctgProcessador(),relat.getPctgMemoriaUso(),relat.getPctgDiscoUso(),relat.getQtdProcessos(),relat.getQtdServicos(),relat.getTemp(),relat.getDataHora());
            connect.getJdbcSql().update("INSERT INTO logs(fkTotem,pctg_processador,pctg_memoria_uso,pctg_disco_uso,qtd_processos,qtd_servicos,temp,data_hora) VALUES (?,?,?,?,?,?,?,?)", relat.getFkTotem(),relat.getPctgProcessador(),relat.getPctgMemoriaUso(),relat.getPctgDiscoUso(),relat.getQtdProcessos(),relat.getQtdServicos(),relat.getTemp(),relat.getDataHora());
...
//                    }
                if (porcentagem1 > 20) {
                    System.out.println("O nível de processamento atingiu: " + porcentagem1);
                ...
                if(porcentagem1>80){
                    ...
                    if (porcentagem3 > 30) {
                        if(porcentagem3>40 && porcentagem3<=60){
                              System.out.println("A memória RAM atingiu: " + porcentagem3); 
                ...
                if (porcentagem5 > 40) {
                    if(porcentagem5>40 && porcentagem5<=60){
                        System.out.println("O armazenamento atingiu: " + porcentagem5);  
                    };
    ...
    private void btnIniciarMonitoramento1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnIniciarMonitoramento1ActionPerformed
        ...
        if(totemCadastrado){
            enviarRelatorio();
            System.out.println("Usuário: " + sessao.getNome() + " Enviou o relatório.");
    ...
    private void btnIniciarMonitoramentoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnIniciarMonitoramentoActionPerformed
        ...
        if(totemCadastrado){
            ...
            if(autorizar == false){
                ...
                System.out.println("Usuário: " + sessao.getNome() + " Iniciou o monitoramento."); 
...
//            logAtual = gerarNovoRelatorio(verificacaoTotem.get(0),comps);
    ...
    private void btnAddTotemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddTotemActionPerformed
        if(totemCadastrado == false){
            ...
            connect.getJdbcSql().update("INSERT INTO totem(fkFilial, ram_total, espaco_disco, processador, data_compra, id_processador, serial_disco, hostname) VALUES (?,?,?,?,?,?,?,?)", sessao.getFkFilial(),ramDouble,discoDouble,comps.getProcessador(),comps.getDataTotem(),comps.getIdProcessador(), comps.getSerialDisco(),comps.getHostname());
    ...
    private void btnSair2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSair2ActionPerformed
        ...
        ativadoDesativado.setForeground(Color.RED);
        ativadoDesativado.setText("Pausado...");
        System.out.println("Usuário: " + sessao.getNome() + " fechou o sistema."
